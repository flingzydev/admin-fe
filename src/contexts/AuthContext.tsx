...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    const messageHandlers = useRef<Set<(message: ParsedMessage) => void>>(new Set());
    ...
    const connect = () => {
        ...
        try {
            ...
            ws.current = new WebSocket(`${API_BASE_URL}/ws?token=Bearer ${authState.accessToken}`);
            ...
            ws.current.onclose = (event) => {
                ...
                if (event.code === WS_ERROR_CODES.CONNECTION_LIMIT) {
                    alert('Connection limit reached. Please close other sessions before continuing.');
                    return;
                }
                ...
                if (reconnectAttempt.current < 5) {
                    ...
                    reconnectTimeout.current = setTimeout(() => queueOperation(connect), delay);
                } else {
                    alert('Unable to establish connection. Please refresh the page to try again.');
                    reconnectAttempt.current = 0;
    ...
    useEffect(() => {
        try {
            ...
            alert('Failed to save authentication state. You may need to login again after refreshing.');
        }
    }, [authState]);
