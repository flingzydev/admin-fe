...
const ChatChannels = ({
  ...
  const fetchChannels = async (before?: string) => {
    try {
      ...
      const response = await fetch(
        `${ADMIN_API_BASE_URL}/users/chat_channels/list?user_id=${user?.id}`,
        {
          method: "POST",
          headers: {
  ...
  const fetchInitialChannels = async () => {
    ...
    if (data) {
      const sortedChannels = data.chat_channel_other_user_pairs.sort(
        (a: ChatChannelOtherUserPair, b: ChatChannelOtherUserPair) =>
          new Date(b.chat_channel.updated_at).getTime() -
          new Date(a.chat_channel.updated_at).getTime(),
      );

      if (sortedChannels.length > 0) {
        lastUpdatedTimeRef.current =
          sortedChannels[sortedChannels.length - 1].chat_channel.updated_at;
      }
  ...
  const fetchMoreChannels = async () => {
    ...
    if (data) {
      ...
      if (newChannels.length > 0) {
        const sortedNewChannels = newChannels.sort(
          (a: ChatChannelOtherUserPair, b: ChatChannelOtherUserPair) =>
            new Date(b.chat_channel.updated_at).getTime() -
            new Date(a.chat_channel.updated_at).getTime(),
        );
        ...
        lastUpdatedTimeRef.current =
          sortedNewChannels[
            sortedNewChannels.length - 1
          ].chat_channel.updated_at;

        setChannels((prev) => {
          const existingIds = new Set(
            prev.map((channel) => channel.chat_channel.id),
          );
          const uniqueNewChannels = sortedNewChannels.filter(
            (channel: ChatChannelOtherUserPair) =>
              !existingIds.has(channel.chat_channel.id),
          );
