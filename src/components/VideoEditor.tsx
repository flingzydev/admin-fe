...
const VideoEditor: React.FC<VideoEditorProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      sliderElement &&
      duration > 0 &&
      !sliderElement.hasAttribute("data-slider-initialized")
    ) {
    ...
    if (sliderElement) {
      ...
      if (slider.noUiSlider) {
        ...
        if (currentTime >= start && currentTime <= end) {
          ...
          const connectElement = sliderElement.querySelector(
            ".noUi-connect",
          ) as HTMLElement;
          if (connectElement) {
  ...
  return (
    <div className="flex flex-col items-center w-full">
      ...
      <div className="mt-8 w-full max-w-[600px]">
        ...
        <div className="text-center mb-4">{formatTime(currentTime)}</div>

        <div className="flex flex-wrap gap-4 justify-center">
          <p className="w-full text-center">
            Start: {formatTime(rangeValues[0])}s | End:{" "}
            {formatTime(rangeValues[1])}s | Rotation: {rotation}°
          </p>
